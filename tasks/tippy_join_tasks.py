...
class TippyJoinTask(commands.Cog):
    ...
    async def _init_db(self):
        try:
            ...
            logger.error(f"[TippyJoin] DB init error: {e}", exc_info=True)
    ...
    async def _get_config(self, guild_id: int) -> Optional[dict]:
        try:
            ...
            logger.error(f"[TippyJoin] Config read error: {e}", exc_info=True)
            return None
    ...
    async def _try_claim(
        ...
        try:
            ...
            logger.error(f"[TippyJoin] Claim error for member {member_id}: {e}", exc_info=True)
            return False
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if not won:
            logger.debug(
                f"[TippyJoin] Skipped member {member.id} in {guild.name} "
                f"(other bot claimed or within cooldown)"
            )
            return

        logger.info(
            f"[TippyJoin] Claimed member {member.id} ({member}) in {guild.name}"
        )
        ...
        for channel_id in cfg['channel_ids']:
            ...
            if channel is None:
                logger.warning(
                    f"[TippyJoin] Channel {channel_id} not found in {guild.name}"
                )
                continue
    ...
    async def _batch_worker(self, channel_id: int, cfg: dict):
        ...
        async with self._channel_locks[channel_id]:
            try:
                ...
                channel = self.bot.get_channel(channel_id)
                if channel is None:
                    logger.warning(f"[TippyJoin] Channel {channel_id} disappeared mid-batch")
                    return
                ...
                try:
                    ...
                    logger.warning(
                        f"[TippyJoin] Missing Send Messages in #{channel} "
                    ...
                    logger.exception(f"[TippyJoin] Send failed in #{channel}: {e}")
                    return
                ...
                logger.info(
                    f"[TippyJoin] Ghost-pinged {len(unique)} member(s) in "
                ...
                logger.exception(
                    f"[TippyJoin] Batch worker crashed for channel {channel_id}: {e}"
                )
            finally:
                # Don't leave a finished Task pinned in _workers forever. Only
                # remove ourselves — never a newer worker that replaced us.
                if self._workers.get(channel_id) is asyncio.current_task():
    ...
    async def _delete_with_retry(
        ...
        for attempt in (1, 2):
            try:
                ...
                logger.warning(
                    f"[TippyJoin] CRITICAL: cannot delete ghost ping in "
                ...
                logger.warning(
                    f"[TippyJoin] Delete attempt {attempt} failed in #{channel}: {e}"
                )
                if attempt == 1:
    ...
    async def _update_stats(self, guild_id: int, members_pinged: int):
        try:
            ...
            logger.error(f"[TippyJoin] Stats update error: {e}", exc_info=True)
    ...
    async def _flush_log(self, guild_id: int):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"[TippyJoin] Log send failed: {e}")
        except asyncio.CancelledError:
            raise
        except Exception as e:
            logger.exception(f"[TippyJoin] Log flush error for guild {guild_id}: {e}")
        finally:
            # Don't leave a finished Task pinned in _log_flush_tasks. Only remove
            # ourselves — never a newer flush task that replaced us.
            if self._log_flush_tasks.get(guild_id) is asyncio.current_task():
                self._log_flush_tasks.pop(guild_id, None)
            ...
            try:
                ...
                async with aiosqlite.connect(SHARED_DB, timeout=10.0) as db:
                    ...
                    if result.rowcount > 0:
                        logger.debug(
                            f"[TippyJoin] Cleaned {result.rowcount} expired claim(s)"
                        )
            except asyncio.CancelledError:
                logger.info("[TippyJoin] Claim cleanup loop cancelled")
                break
            except Exception as e:
                logger.error(f"[TippyJoin] Cleanup error: {e}", exc_info=True)


async def setup(bot):
    await bot.add_cog(TippyJoinTask(bot))
    logger.info("✅ TippyJoinTask cog loaded")
