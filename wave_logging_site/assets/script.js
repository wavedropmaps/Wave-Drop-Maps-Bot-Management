...
const AUDIT_EVENT_CONFIG = {
  ...
  // maintenance
  vacuum_complete:        { icon: "🔧",  label: "VACUUM Complete",       color: "#57f287" },
  vacuum_failed:          { icon: "❌",  label: "VACUUM Failed",         color: "#ed4245" },
  backup_complete:        { icon: "💾",  label: "Backup Complete",       color: "#57f287" },
  backup_pruned:          { icon: "🗑️",  label: "Backup Pruned",         color: "#8e9297" },
};
...
const TABS = {
  manager: [
    ...
    { label: "Surge Routes",                      actions: ["surge_routes"] },
    ...
    { label: "Maintenance",                       actions: ["maintenance"] },
...
const CACHE_MAX_AGE_MS = 10 * 60 * 1000; // stale-while-revalidate window

// ──────────────────────────────────────────────────────────────
// IndexedDB cache  (persistent across page reloads)
// ──────────────────────────────────────────────────────────────
const _IDB_NAME    = "wave-logging-cache";
const _IDB_VER     = 1;
const _IDB_STORE   = "events";
let   _idb         = null;
...
// In-memory layer on top of IDB — avoids repeated async IDB reads
const memCache = new Map();
// key: "section:category"  →  { events: [...], fetchedAt: timestamp }

function _cacheKey(section, category) { return `${section}:${category}`; }

function isCacheStale(entry) {
  return !entry || !entry.fetchedAt || (Date.now() - entry.fetchedAt) > CACHE_MAX_AGE_MS;
}

async function getCached(section, category) {
  const key = _cacheKey(section, category);
...
async function setCached(section, category, events) {
  const key   = _cacheKey(section, category);
  const entry = { events, fetchedAt: Date.now() };
  memCache.set(key, entry);
  await _idbSet(key, entry);    // fire-and-forget
}

// ──────────────────────────────────────────────────────────────
// Prefetch orchestrator  (priority queue, 6 concurrent fetches)
// ──────────────────────────────────────────────────────────────
const prefetcher = {
  queue: [],
  running: 0,
  ...
  enqueue(section, category, priority = 0) {
    const key = _cacheKey(section, category);
    const cached = memCache.get(key);
    if (cached && !isCacheStale(cached)) return;         // already fresh
    const existing = this.queue.find(i => i.key === key);
    if (existing) {
      if (priority > existing.priority) { existing.priority = priority; this._sort(); }
      return;
    }
    this.queue.push({ key, section, category, priority });
    this._sort();
    this._drain();
  },
  ...
  _drain() {
    while (this.running < this.MAX_CONCURRENT && this.queue.length > 0) {
      ...
      this.running++;
      fetchEventsForCategory(item.section, item.category)
        .then(evts => setCached(item.section, item.category, evts))
        .catch(() => {})
...
// ── Usage-weighted prefetch priority (localStorage) ─────────────
function recordClick(section, category) {
  try {
    const data = JSON.parse(localStorage.getItem("wl_clicks") || "{}");
    const k = _cacheKey(section, category);
    data[k] = (data[k] || 0) + 1;
    localStorage.setItem("wl_clicks", JSON.stringify(data));
  } catch {}
}
...
function buildPrefetchQueue() {
  try {
    const data = JSON.parse(localStorage.getItem("wl_clicks") || "{}");
    const items = [];
    for (const [sectionKey, tabs] of Object.entries(TABS)) {
      for (const group of tabs) {
        for (const category of group.actions) {
          items.push({ section: sectionKey, category, priority: data[_cacheKey(sectionKey, category)] || 0 });
        }
      }
    }
    items.sort((a, b) => b.priority - a.priority);
    for (const item of items) prefetcher.enqueue(item.section, item.category, item.priority);
  } catch {}
}

function prefetchSection(sectionKey, extraPriority = 10) {
  const tabs = TABS[sectionKey];
  if (!tabs) return;
  for (const group of tabs) {
    for (const category of group.actions) prefetcher.enqueue(sectionKey, category, extraPriority);
  }
}

function scheduleAdjacencyPrefetch(tabsKey, groupIdx, bot) {
  const tabs = TABS[tabsKey];
  if (!tabs) return;
  const section = bot || "server";
  [-1, 1, 2].forEach(offset => {
    const g = tabs[groupIdx + offset];
    if (!g) return;
    for (const cat of g.actions) prefetcher.enqueue(section, cat, 5);
  });
}
...
// ──────────────────────────────────────────────────────────────
const state = {
  currentSection: null,    // "server" | "bots"
  currentBot: null,        // "manager" | "logistics"  (when section === "bots")
  currentGroupIdx: null,   // index into TABS[activeTabsKey]
...
const el = {
  ...
  // tab containers
  tabBarServer:    () => $("#tab-bar-server"),
  botTabRows:      () => $("#bot-tab-rows"),
  botRowManager:   () => $("#bot-row-manager"),
  botRowLogistics: () => $("#bot-row-logistics"),
  tabBarManager:   () => $("#tab-bar-manager"),
  tabBarLogistics: () => $("#tab-bar-logistics"),
  // event stream
...
document.addEventListener("DOMContentLoaded", () => {
  // Tile clicks + hover-intent prefetch
  document.querySelectorAll(".tile").forEach(tile => {
    tile.addEventListener("click", () => openSection(tile.dataset.target));
    tile.addEventListener("mouseenter", () => {
      if (tile.dataset.target === "bots") {
        prefetchSection("manager", 20);
        prefetchSection("logistics", 20);
      } else {
        prefetchSection("server", 20);
      }
    });
  });
  ...
  el.refreshBtn().addEventListener("click", () => {
    if (state.currentGroupIdx != null) loadGroup(state.currentGroupIdx, state.currentChild, state.currentBot);
  });
  el.loadMoreBtn().addEventListener("click", showMoreRows);

  loadGuilds().then(() => {
    renderLandingStats();
    buildPrefetchQueue();   // background prefetch, usage-weighted
  });
});
...
function openSection(sectionSlug) {
  if (!["server", "bots"].includes(sectionSlug)) return;
  ...
  if (sectionSlug === "server") {
    el.botTabRows().classList.add("hidden");
    el.tabBarServer().classList.remove("hidden");
    renderTabsIntoBar("server", el.tabBarServer(), null);
    loadGroup(0, null, null);
  } else {
    // bots: show two labeled rows
    el.tabBarServer().classList.add("hidden");
    el.botTabRows().classList.remove("hidden");
    renderTabsIntoBar("manager",   el.tabBarManager(),   "manager");
    renderTabsIntoBar("logistics", el.tabBarLogistics(), "logistics");
    // auto-select first tab of Wave Manager
    loadGroup(0, null, "manager");
  }
...
// Tabs
// ──────────────────────────────────────────────────────────────
// renderTabsIntoBar(tabsKey, barEl, bot)
//   tabsKey — "server" | "manager" | "logistics"  (key into TABS)
//   barEl   — the <nav> element to populate
//   bot     — "manager" | "logistics" | null  (passed to loadGroup)
function renderTabsIntoBar(tabsKey, barEl, bot) {
  barEl.innerHTML = "";
  TABS[tabsKey].forEach((group, idx) => {
    if (group.actions.length === 1) {
      barEl.appendChild(makePlainTab(group, idx, bot));
    } else {
      barEl.appendChild(makeDropdownTab(group, idx, bot));
    }
  });
  document.addEventListener("click", closeAllDropdownsOnOutsideClick);
}

function makePlainTab(group, idx, bot) {
  const btn = document.createElement("button");
  btn.className = "tab";
  btn.dataset.groupIdx = String(idx);
  btn.textContent = group.label;
  btn.addEventListener("click", () => loadGroup(idx, null, bot));
  return btn;
}

function makeDropdownTab(group, idx, bot) {
...
function setActiveTab() {
  ...
  // Clear bot-row highlighting
  const rowMgr = el.botRowManager();
  const rowLog = el.botRowLogistics();
  if (rowMgr) rowMgr.classList.remove("active-row");
  if (rowLog) rowLog.classList.remove("active-row");

  if (state.currentGroupIdx == null) return;

  // Determine which bar element to scope the selector to
  let barId;
  if (state.currentSection === "server") {
    barId = "#tab-bar-server";
  } else if (state.currentBot === "manager") {
    barId = "#tab-bar-manager";
    if (rowMgr) rowMgr.classList.add("active-row");
  } else {
    barId = "#tab-bar-logistics";
    if (rowLog) rowLog.classList.add("active-row");
  }

  const groupNode = document.querySelector(
    `${barId} .tab[data-group-idx="${state.currentGroupIdx}"], ` +
    `${barId} .tab-dropdown[data-group-idx="${state.currentGroupIdx}"] .tab-dropdown-btn`
  );
  if (groupNode) groupNode.classList.add("active");

  if (state.currentChild) {
    const childNode = document.querySelector(
      `${barId} .tab-dropdown[data-group-idx="${state.currentGroupIdx}"] ` +
    ...
    const allItem = document.querySelector(
      `${barId} .tab-dropdown[data-group-idx="${state.currentGroupIdx}"] .tab-dropdown-item-all`
    );
...
// ──────────────────────────────────────────────────────────────
// Data loading  (stale-while-revalidate + smart caching)
// ──────────────────────────────────────────────────────────────
/**
 * Open a tab group.
 *   groupIdx  — index into TABS[tabsKey]
 *   childSlug — narrows to one action within the group (null = all)
 *   bot       — "manager" | "logistics" | null (null = server)
 *
 * Flow:
 *   1. Serve stale cached events immediately (instant render).
 *   2. Fetch fresh data from network in parallel.
 *   3. Re-render with fresh results (silent update if stale was shown).
 */
async function loadGroup(groupIdx, childSlug = null, bot = null) {
  // Resolve which TABS key and data-folder section to use
  const tabsKey = (state.currentSection === "bots")
    ? (bot || state.currentBot || "manager")
    : "server";
  const section = (tabsKey === "server") ? "server" : tabsKey;  // data folder

  const group = TABS[tabsKey][groupIdx];
  if (!group) return;

  state.currentGroupIdx = groupIdx;
  state.currentChild    = childSlug;
  if (bot) state.currentBot = bot;
  state.visibleCount = PAGE_SIZE;
  setActiveTab();

  const actionsToLoad = childSlug ? [childSlug] : group.actions;
  actionsToLoad.forEach(a => recordClick(section, a));

  // ── 1. Serve stale cache immediately ──────────────────────────
  const cachedResults = await Promise.all(actionsToLoad.map(a => getCached(section, a)));
  const staleEvents = [];
  for (const hit of cachedResults) if (hit && hit.events) staleEvents.push(...hit.events);
  const hasStale = staleEvents.length > 0;

  if (hasStale) {
    staleEvents.sort((a, b) => (b.timestamp || "").localeCompare(a.timestamp || ""));
    state.allEvents = staleEvents;
  ...
  try {
    const perAction = await Promise.all(
      actionsToLoad.map(a => fetchEventsForCategory(section, a))
    );
    actionsToLoad.forEach((a, i) => { if (perAction[i]) setCached(section, a, perAction[i]); });
    const merged = [];
    for (const list of perAction) if (list) merged.push(...list);
    merged.sort((a, b) => (b.timestamp || "").localeCompare(a.timestamp || ""));
    state.allEvents = merged;
  } catch (err) {
    console.error("[wave-logging] loadGroup error:", err);
    if (!hasStale) state.allEvents = [];
  }
  ...
  // ── 3. Adjacency prefetch ──────────────────────────────────────
  scheduleAdjacencyPrefetch(tabsKey, groupIdx, section === "server" ? null : section);
}
...
function pickSummaryHint(event) {
  ...
  // Maintenance
  if (cat === "maintenance") {
    if (event.action === "vacuum_complete" && det.elapsed_seconds != null)
      return `completed in ${det.elapsed_seconds}s`;
    if (event.action === "vacuum_failed" && det.error)
      return truncate(det.error, 80);
    if (event.action === "backup_complete")
      return `${det.folder || ""}${det.files_backed_up != null ? ` · ${det.files_backed_up} files` : ""}`.trim();
    if (event.action === "backup_pruned" && det.folder)
      return det.folder;
  }
...
function buildDetailView(container, event) {
  ...
  if (cat === "maintenance") {
    const card = document.createElement("div");
    card.className = "detail-kv-block";
    if (det.elapsed_seconds != null) card.appendChild(makeKV("Duration", det.elapsed_seconds + "s"));
    if (det.folder)                  card.appendChild(makeKV("Folder", det.folder, { code: true }));
    if (det.files_backed_up != null) card.appendChild(makeKV("Files backed up", det.files_backed_up));
    if (det.files_skipped != null && det.files_skipped > 0)
                                     card.appendChild(makeKV("Files skipped", det.files_skipped, { color: "#faa81a" }));
    if (det.skipped_list && det.skipped_list.length)
                                     card.appendChild(makeKV("Skipped", det.skipped_list.join(", "), { code: true }));
    if (det.error)                   card.appendChild(makeKV("Error", det.error, { color: "#ed4245" }));
    if (card.children.length)        container.appendChild(makeSection("Details", card));
  }
...
function startAutoRefresh() {
  ...
  state.autoRefreshTimer = setInterval(() => {
    if (state.currentGroupIdx != null) {
      loadGroup(state.currentGroupIdx, state.currentChild, state.currentBot);
    }
  }, AUTO_REFRESH_MS);
}
