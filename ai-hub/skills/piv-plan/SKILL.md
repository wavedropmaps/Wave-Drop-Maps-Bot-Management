...
Use the **Write tool** to create the file `plans/<feature-slug>-plan.md` — this is a required deliverable, not optional. Do **NOT** just print the plan in your response; `/implement` reads it from disk, so the file must exist. 

**CRITICAL TDD RULE:** If this is a BUG FIX, Task 1 MUST be to write a failing test script that reproduces the bug. Do not write the fix until the failing test exists.
