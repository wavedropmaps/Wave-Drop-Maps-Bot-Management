...
from tasks.unified_weekly_loop import (
    post_duty_info_embeds,
    build_duty_info_embed,
    perform_full_scan,
    _build_unified_hub_payload,
    DUTY_INFO_CONFIG,
    _DUTY_INFO_FOOTER_TAG,
    find_staff_for_duty,
    find_general_staff,
    scan_duty_all_guilds,
    scan_modlog_all_guilds,
    scan_messages_all_guilds_optimized,
    scan_reviews,
    cache_stat,
    RateLimitTracker,
    rate_limit_tracker,
...
# Inert cog — do NOT start the old loop. unified_weekly_loop handles everything.
class DutiesScansCog:
    """DEPRECATED — use UnifiedWeeklyLoop from tasks.unified_weekly_loop instead."""
    pass


async def setup(bot):
    logger.info("duties_scan.py is DEPRECATED — all logic in tasks/unified_weekly_loop.py")
"""
...
def build_duty_info_embed(duty_type: str) -> Optional[discord.Embed]:
    ...
    embed.add_field(
        ...
        value=(
            "Get a ❌ **Bad** rank? You lose **200 VBucks** from your balance.\n"
...
# ==================== HITL REVIEW SCAN (cross-bot, from bot_logs) ====================

def _iso_z(dt: datetime) -> str:
    """Format a datetime to match how global_logger stores timestamps:
    'YYYY-MM-DDTHH:MM:SSZ' (UTC, second precision). Lexicographic string
    comparison on this format is chronologically correct."""
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)
    return dt.astimezone(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z")


async def scan_reviews(start_datetime: datetime, end_datetime: datetime) -> Dict[int, int]:
    """
    Count HITL proof-review clicks per staff member for the report window.

    The Wave Logistics Bot writes rows to bot_logs (this same DB) under
    category='hitl_review'. We count ONLY rows with action='review_completed' —
    one per HITL proof a staff member actually resolved (a terminal verdict, or a
    class-pick that resolved the proof). Intermediate class-pick handoffs log
    action='review_step_completed' and are deliberately NOT counted, so a single
    proof that bounces through several confused steps counts once, not once per
    step. We do a single indexed read (idx_bot_logs_bot_cat), bucket by actor id,
    and return {user_id: review_count}. No Discord API calls. Window-scoped, so it
    resets each report.

    ROOT CAUSE (identified 2026-06-21): As of this date, bot_logs has ZERO
    hitl_review rows — reviews always show 0 on the weekly page. Investigation
    confirmed the Logistics bot IS configured to write to this DB (its
    utils/global_logger.py sets _DB_FILE = "../Wave Management Bot/bot_database.db"),
    but the calls in Tasks/proof_automation_tasks.py pass
      bot=message.guild.me   (a discord.Member object)
    instead of the expected string. SQLite cannot bind a Member to a TEXT column,
    so the INSERT raises InterfaceError, which is silently swallowed by the
    NullHandler logger. No hitl_review rows ever land here.

    FIX REQUIRED IN THE LOGISTICS BOT (Tasks/proof_automation_tasks.py):
    Remove the `bot=` argument from all four log_event() calls that emit
    category='hitl_review' so they fall back to the default BOT_NAME='logistics':
        # WRONG (causes silent INSERT failure):
        await log_event(bot=message.guild.me, category='hitl_review', ...)
        # CORRECT (uses default 'logistics' string):
        await log_event(category='hitl_review', ...)
    The four call sites are roughly at lines 597, 637, 674, and 1484 of
    Tasks/proof_automation_tasks.py in the Wave Logistics Bot repo.
    This file (duties_scan.py) requires no changes once the Logistics bot is fixed.
    """
    import aiosqlite
    import database
    ...
    try:
        async with aiosqlite.connect(database.DB_FILE) as conn:
            ...
            cursor = await conn.execute(
                ...
                WHERE category = 'hitl_review'
                  AND action = 'review_completed'
    ...
    logger.info(f"  ✅ reviews: {len(counts)} users with HITL reviews ({sum(counts.values())} total)")
    return counts
...
def _duty_rank(count: int, duty_type: str) -> tuple:
    ...
    # req thresholds (role duty retired)
...
def _build_unified_hub_payload(
    ...
    for uid in all_uids:
        ...
        if uid in general_staff_ids:
            ...
            reviews      = int(all_stats.get('reviews', {}).get(uid, 0))
...
async def perform_full_scan(bot):
    ...
    try:
        ...
        # ── 1. REQ ───────────────────────────────────────────────────────────
        logger.info("\n" + "─" * 60)
        logger.info("📋 [1/3] SCANNING: req (Map Request Helper)")
        logger.info("─" * 60)
        ...
        # ── 4. MODLOG ────────────────────────────────────────────────────────
        logger.info("\n" + "─" * 60)
        logger.info("📋 [2/3] SCANNING: modlog (Mod Commands)")
        logger.info("─" * 60)
        ...
        # ── 5. MESSAGES ──────────────────────────────────────────────────────
        logger.info("\n" + "─" * 60)
        logger.info("📋 [3/3] SCANNING: message (Messages)")
        logger.info("─" * 60)
        ...
        # ── HITL REVIEWS (cross-bot, read from bot_logs — no API) ─────────────
        logger.info("\n" + "─" * 60)
        logger.info("📋 SCANNING: reviews (HITL proof reviews from Logistics bot)")
        logger.info("─" * 60)
        review_counts = await scan_reviews(start_datetime, end_datetime)
        all_stats['reviews'] = review_counts
        logger.info(f"  ✅ reviews scan complete: {len(review_counts)} users with reviews")
        ...
        try:
            ...
            for _uid in _all_export_uids:
                try:
                    ...
                    _raw = str(_u.display_avatar.url) if _u and _u.display_avatar else None
                    _avatar_cache[int(_uid)] = _raw.replace('.gif?', '.png?') if _raw else None
            ...
            for duty_type in ['req', 'modlog', 'message']:
...
class DutiesScansCog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        logger.info(
            f"📋 Leaderboard channels: req={LEADERBOARD_CHANNELS['req']}"
        )
